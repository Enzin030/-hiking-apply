...
5. 改完 UI 要用瀏覽器實際跑過再回報（程序見 `institution\core.md` §3.3），不以「程式碼看起來對」代替驗證。
   **驗收要檢查哪些項目見下面「改完要自己驗的事」**。
6. 介面走極簡：強調資訊層級、留白，減少視覺雜訊。

## 改雛形怎麼做（不分模式，照證據判斷）

**沒有「先分類再動手」這一關。** 收到任務就做，界線與驗收由下面兩件事決定。

### 預設界線

- **改動範圍＝被點名的頁面／元件**，以及為完成它所必需的 token、共用元件、樣式。
  這些**直接做，不必事前核准**——收手時列出來說明理由即可。
  **本條為專案層明示，優先於 `institution\core.md` §3.2「未點名不動」**（依 core.md §0）。
- 範圍外的頁面不動。看到別處可以順手改善，記入 `decisions.md` 當候選，不在本次夾帶。
- **使用者只給感覺**（太擠、看起來不對）而沒給做法時，**自己決定一個合理做法直接改**，
  在回報時說明改了什麼、為什麼這樣改。不要先來回確認才動手。
  真的有兩三種明顯不同的走向且結果差很多，才在回報中並陳讓使用者挑。
- **不改外觀的遷移／清理**（階段 4 剩餘工作）是唯一的例外：該情境的目標就是
  「與改版前行為相同」，額外要求像素／computed 零差異，**看到可改善也不改、不為統一而動 DOM**。
  完整敘述見踩坑總表第四節「遷移期的兩條界線」——**那兩條只適用這個情境**，
  有意改外觀時不套用。

### 改完要自己驗的事

事前的範圍協商換成事後的變動證明，所以下面這幾項是硬要求，不必使用者逐次重述：
...
5. 新增的 token／元件／頁面專用 class 列一份清單，附理由。
...
| 規格文件（39 份，含各頁規格與 `00_系統功能設計說明.md`；**已有實質內容**，部分業務數值仍標 `[待確認]`，動內容前先讀該檔的溯源欄位） | `02_Spec/` |
| 早期規劃草稿（PRD、實作方針、DB 清單、UI/UX 提案、簡報稿） | `.scratch/docs-舊規劃/`——**不進版控**，僅供回查 |
| 雛形（頁面、元件、樣式、圖檔） | `05_Prototype/` |
| 來源資料、機關提供檔案 | `01_Raw_Input/` |
| 資料表結構（由 `scripts/gen-schema-from-csv.js` 產生） | `03_Schema/` |
| 舊站 code 分析（規格的溯源依據之一，標的為**測試機**） | `04_Handover/` |
...
- **舊 React 版 `components/*.jsx` 已於階段 4 全數刪除**（現為 0 個）。
  舊紀錄或腳本若提到 `.jsx`，一律是失效引用，不要據此去找檔案。
- 樣式見下面「樣式歸屬」一節
...
- 不要另立 `styles/` 之類的第二個樣式目錄，也不要在頁面寫 `<style>` 或 inline `style`
  （動態綁定的 `:style` 除外）。
- **舊的 `05_Prototype/styles/`（含 `shared.css`）已於階段 4 整個刪除**，
  上表四層是唯一的樣式來源。
...
其中**第四節「遷移期的兩條界線」只適用「不改外觀的遷移／清理」**（見上面「預設界線」末項）——
有意改外觀時不套用；其餘各節（十類靜默失敗、對照設計檔以畫面為基準、驗收盲點）一律適用。
...
同日依實測修正處理順序）。遇到 `D:\git-dirs\登山一站式.git\index.lock` 擋住操作時：

**第一步一律是重試，不是判定孤兒。**

```bash
for i in 1 2 3 4 5; do
  if <git 指令> 2>/dev/null; then break; fi
  sleep 3
done
```

**重試多次仍在，才進入孤兒判定**，依序確認兩件事：
...
**⚠ 為什麼重試要排在孤兒判定之前**（2026-09-16 實測更正）：
本專案**有多個 session 併行寫入**，而併行寫入者的 lock 是**瞬時**的——
同日第 3 次遇到時觀察到它**會自己消失**（連續三次取樣皆已不存在），
所以那不是孤兒檔，是別人正在寫。**上面那兩項檢查在併行情境下會誤判成孤兒**：
獨佔開啟能成功、唯讀 git 行程一大堆卻沒有寫入型的，兩項都會「成立」。
同日前兩次（10:27:53、11:25:11，皆 0 bytes）依舊版判準刪除、沒有可見損害，
**但那是運氣好，不是判準對**——刪掉別人正在用的 lock 可能讓對方的寫入半途失敗。

本專案實測這個 lock 會反覆出現（2026-09-15 00:29、01:11，2026-09-16 09:24、
10:27、11:25 各一次），每次都要重新走一遍流程，
不能因為「上次是孤兒」就直接刪。

**〔觀察，非規則〕2026-09-16 共四次滯留 lock（10:27:53、11:25:11、12:42:49、13:28:42）。**
前三次出現在本 session 自己的 commit 前後，曾據此懷疑是自己的工具鏈造成；
**但第四次（13:28:42、滯留 100 分鐘）出現在本 session 完全閒置的期間**，
推翻了那個歸因。目前只能說：**滯留型 lock 的來源不明**，與「誰在動」無明顯相關。
上面「併行寫入者的 lock 是瞬時的」那段解釋了**瞬時**lock（會自己消失的那種），
但**滯留型**的來源目前無法歸因——三次都是 0 bytes、滯留 6～7 分鐘、
兩項檢查皆成立。樣本不足，**本節規則不因此改動**；若下次仍是同一模式，
「併行寫入者造成」這個歸因就要重寫。

同日另有一個相關的誤判：`git commit` 連 6 次失敗被判為搶鎖，實因 message 檔
未被寫出，而重試迴圈用 `2>/dev/null` 吞掉了 `could not read log file`。
**重試機制在最終失敗時必須輸出原始錯誤**，否則會誘發錯的補救動作
（本例是去刪一個其實無關的 lock）。見
`_knowledge\shared\tech\vue3-global-build-遷移踩坑.md` 六之 5b。
...
**有意改外觀時同樣沒有「與改版前相同」可比對**——那正是界線改由「被點名的範圍」界定、
並要求改動範圍外的頁面須零差異的原因。
...
- 領域詞彙／業務規則的正本在知識庫，`CONTEXT.md` 只留路由——不要在兩處各存一份。
  （`CONTEXT.md` 是給 diagnose／tdd 等 skill 讀的領域摘要，與本檔分工：本檔管怎麼做，該檔管領域詞彙。）
