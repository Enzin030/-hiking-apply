/* ============================================================
   ...
   導覽字級 16px 不可改大：18px 在 1280 容器內塞不下同一行。
...
window.thComponents["th-header"] = {
  ...
  template: `
    <header class="w-full bg-white sticky top-0 z-50 shadow-[0_2px_10px_rgba(0,0,0,0.05)] border-b border-slate-100 py-3">
        ...
        <div class="flex flex-wrap justify-between items-center gap-y-2">
          ...
          <a href="index.html"
            ...
            <span class="font-serif font-extrabold text-base sm:text-xl lg:text-2xl text-slate-800 tracking-wide mt-1 min-w-0 lg:whitespace-nowrap">
              <span class="text-lg sm:text-2xl lg:text-3xl">臺灣<span class="text-[#587a68]">登山申請</span></span>一站式服務網
            </span>
          </a>

          <!-- 桌機（≥1280px）：工具列 ＋ 水平主導覽 -->
          <div class="flex-col items-end gap-3 hidden xl:flex">
            <div class="flex items-center gap-3 text-[14px] text-slate-500">
              <template v-for="u in utility" :key="u.label">
                <a v-if="u.url" :href="u.url"
                   ...
                   class="hover:text-[#587a68] transition">{{ u.label }}</a>
              ...
              <div class="th-lang-wrapper" ref="langWrapper">
                <button type="button" class="th-lang-btn hover:text-[#587a68] transition"
                        @click="langOpen = !langOpen"
                        :aria-expanded="langOpen ? 'true' : 'false'" aria-haspopup="true">
                  <i class="ph ph-globe text-[14px] relative top-[1px]"></i>
                  <span>{{ currentLangObj.label }}</span>
                  <i :class="['fa-solid fa-chevron-down text-[10px] transition-transform', { 'rotate-180': langOpen }]"></i>
                </button>
                <div v-if="langOpen" class="th-lang-dropdown">
                  <button v-for="lang in languages" :key="lang.key" type="button"
                    ...
                    <i v-if="currentLang === lang.key" class="fa-solid fa-check text-xs text-[#587a68]"></i>
                  </button>
                </div>
              </div>

              <div class="w-[1px] h-3 bg-slate-300"></div>
              <button class="hover:text-[#587a68] transition flex items-center gap-1.5 ml-1">
                <i class="ph-bold ph-magnifying-glass text-[15px] relative top-[1px]"></i>
              </button>
            </div>
            ...
            <nav class="flex items-center gap-4" aria-label="主要導覽">
              <template v-for="n in nav" :key="n.key">
                <a v-if="n.url" :href="n.url"
                   :class="['font-medium transition text-[16px] whitespace-nowrap',
                            active === n.key ? 'text-[#587a68]' : 'text-slate-600 hover:text-[#587a68]']">
                  {{ n.label }}
                </a>
                <th-todo-link v-else :label="n.label"
                              extra-class="th-todo-link-nav text-[16px] whitespace-nowrap"></th-todo-link>
              </template>
            </nav>
          </div>
