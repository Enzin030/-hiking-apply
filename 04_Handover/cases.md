...
- **2026-09-18 補記（掃描範圍）**：上面雪霸的「零命中」只掃了 `App_code/SheipaWebService.cs` 一支。
  雪霸的申請頁 `apply_1_3.aspx.cs`（9,193 行）有另一組名額相關的讀取：
  `node`（7 處）、`node_arrive`（5）、`node_one`（4）、`node_two`（4）、`node_closedate`（4）、`fixedclimb`（8）、
  以及「承載量」字樣 31 行、「已通過」字樣。
  → 雪霸**有**以節點為單位的承載量檢核（與 `系統概覽.md`「承載量存在 `node` 表」及排程
  `排程-雪霸-檢查是否有超出承載量的宿營地` 一致），~~**本條「沒有候補／遞補」的結論不受影響**~~〔2026-09-18 17:10 更正，見下一則〕。
  **這組表的實際演算法尚未讀**（`node_one`／`node_two` 各代表什麼、如何比對承載量），`[待確認]`。
  來源：`.scratch/outputs/正式站申請流程盤點/F1三家比對.md` Q3，測試機程式碼。
- **2026-09-18 17:10 更正上一則補記**：「本條『沒有候補／遞補』的結論不受影響」**不成立**。
  讀完 `apply_1_3.aspx.cs` 後，**雪霸申請頁本身有候補機制**，只是不在 `SheipaWebService.cs`：
  - 每晚每個宿營地：承載量＝`node.holidaynum`（該日在 `HolidaySet` 為假日）或 `node.generalnum`（營地改用 `holidaynumt`／`generalnumt`）；
    剩餘＝承載量 −（`booking` 已核可 `chk=1` ＋ 待審 `chk=0` 且申請案 `status in (0,1)`）；
    需求＝隊伍人數（床，`roomtype=0`）或 ⌈人數／`roompeople`⌉（營地，`roomtype=1`）（`:3486-3640`）
  - 送件時（`RuleSet.loadingtype = 1` 才檢查，`:5757`）：任一晚需求 > 剩餘 → 整件 `status = 3`（候補），
    **「不夠全轉候補不塞床位」**（`:6550` 註解）；任一晚需求 > 剩餘 ＋ `RuleSet.bakroomnum`（營地為 `bakcampnum`）→
    擋下送件，alert「床位不足且候補已滿，建議變更行程」（`:5767`）
  - **前面有人候補就跟著候補**：同宿營地同日已有未配到床位的申請（`bookingnumok = 0`），本件也轉 `status = 3`（`:5776-5798`）
  - 畫面上的剩餘說明**算完即被清空**（`:3623`、`:3629`），只顯示「實際順位以送出後為準」——申請人看不到剩餘數量〔待實走確認〕
  - `node_one`／`node_two` 不是名額：是**特殊路線**判定（單日長程來回／首日長程重裝），命中只顯示提醒文字（`:1280-1348`、`:2657`）；
    `node_arrive` 是次路線各節點可到達的下一站、`node_closedate` 是節點關閉期間（`:1042`、`:1113`），都屬路線規劃器
  - 程式裡有 `statustype == "6"`（抽籤）的分支，但**雪霸沒有任何地方把它設成 6**，推定為從玉山複製來的死分支
  → 三家都有「額滿後的處理」，只是機制不同：玉山抽籤、太魯閣排程候補、**雪霸送件當下即時判定候補＋後補額度**。
  **未找到**：雪霸的「遞補」（有人取消後候補者補上，`notice_a7` 所述）在申請頁沒有實作，可能在後台或排程，`[待確認]`。
  詳見 `.scratch/outputs/正式站申請流程盤點/機關確認事項.md`。規則值來自測試機資料表，數值待確認。
